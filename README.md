...
- **Task Prioritization (NEW)**:
    ...
    - Tasks and Subtasks now include a `Priority` field (e.g., `Low`, `Medium`, `High`, `Critical`).
        
    - Allows filtering and sorting tasks and subtasks by priority.
        
- **Task Categories/Tags (NEW)**:
    
    - Implement a tagging system to categorize tasks (e.g., "Work", "Personal", "Urgent", "Project X").
        
    - Allows multiple tags per task.
        
    - Enables filtering tasks by one or more tags.
        
    - Admin users can manage (create, update, delete) tags via a dedicated endpoint. Tags cannot be deleted if associated with tasks.
...
2. **Navigate to the Solution Directory**: Ensure you are in the root directory of your solution, e.g., `TaskManagementSolution/`.
    
3. **Install NuGet Packages**: Open your terminal or command prompt and run the following commands for each project:
...
4. **Update Database Connection String**: Open `TaskManagementApi.Presentation/appsettings.json` and ensure your `DefaultConnection` string points to your SQL Server instance.
...
5. **Apply Database Migrations and Seed Data**: From the `TaskManagementSolution/` directory, run the following commands to create the database schema and seed initial data (roles, users, tasks, subtasks, **tags**, and **priorities**):
    ...
    # NEW: Run this migration for Priority and Tags
    dotnet ef migrations add AddPriorityAndTags -p TaskManagementApi.Infrastructure -s TaskManagementApi.Presentation
    ...
    - **New**: Seeded tags like "Frontend", "Backend", "Testing", "Urgent", etc.
        
    - **New**: Existing tasks and subtasks will have default priorities assigned.
        
6. **Run the API**: From the `TaskManagementSolution/` directory:
...
- `GET /api/Tasks`
    ...
    - Get all tasks. Supports filtering (`search`, `isCompleted`, `dueDateFrom`, `dueDateTo`, `includeDeleted`, `includeNotified`, **`priority`**, **`tags`**), sorting (`sortBy`, `sortOrder`), and pagination (`pageNumber`, `pageSize`).
...
- `GET /api/Tasks/MyAssignedTasks` (Requires `Admin` Role)
    ...
    - Specifically for admins to get tasks assigned _to_ them. Supports the same query parameters as `GET /api/Tasks`.
...
- `POST /api/Tasks`
    ...
    - Request Body: `{ "title": "string", "description": "string", "dueDate": "datetime?", "isNotificationEnabled": "boolean", "notificationDateTime": "datetime?", "assignedToUserId": "string?", "priority": "Low|Medium|High|Critical", "tags": ["string"] }`
        
- `PUT /api/Tasks/{id}`
    
    - Update an existing task. Admins can reassign tasks using `assignedToUserId`.
        
    - Request Body: `{ "id": "int", "title": "string", "description": "string", "isCompleted": "boolean", "dueDate": "datetime?", "isNotificationEnabled": "boolean", "notificationDateTime": "datetime?", "assignedToUserId": "string?", "priority": "Low|Medium|High|Critical", "tags": ["string"] }`
...
- `GET /api/Tasks/{parentTaskId}/SubTasks`
    ...
    - Get all subtasks for a specific parent task. Supports filtering (`search`, `isCompleted`, `dueDateFrom`, `dueDateTo`, `includeDeleted`, **`priority`**), sorting (`sortBy`, `sortOrder`), and pagination (`pageNumber`, `pageSize`).
...
- `POST /api/Tasks/{parentTaskId}/SubTasks`
    ...
    - Request Body: `{ "title": "string", "description": "string", "dueDate": "datetime?", "priority": "Low|Medium|High|Critical" }`
        
- `PUT /api/Tasks/{parentTaskId}/SubTasks/{id}`
    
    - Update an existing subtask.
        
    - Request Body: `{ "id": "int", "title": "string", "description": "string", "isCompleted": "boolean", "dueDate": "datetime?", "priority": "Low|Medium|High|Critical" }`
...
### Tag Management (Requires `Admin` Role for CUD)

- `GET /api/Tags`
    
    - Get all available tags in the system. (Accessible by any authenticated user)
        
- `GET /api/Tags/{id}`
    
    - Get a specific tag by ID. (Accessible by any authenticated user)
        
- `POST /api/Tags` (Requires `Admin` Role)
    
    - Create a new tag.
        
    - Request Body: `{ "name": "string" }`
        
- `PUT /api/Tags/{id}` (Requires `Admin` Role)
    
    - Update an existing tag.
        
    - Request Body: `{ "id": "int", "name": "string" }`
        
- `DELETE /api/Tags/{id}` (Requires `Admin` Role)
    
    - Delete a tag. Will fail if the tag is currently associated with any tasks.
...
3. In Swagger UI, click the "Authorize" button (top right), select "Bearer", and paste your JWT token in the format `Bearer YOUR_JWT_TOKEN_HERE`. (e.g., `Bearer eyJhbGciOiJIUzI1Ni...`)
...
- **Admin**: Full access to all functionalities, including user and role management, task deletion/restoration, assigning tasks to any user, and **full tag management**.
